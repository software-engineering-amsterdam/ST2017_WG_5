...
exMHelper :: Integer -> Integer -> Integer -> Integer -> Integer
exMHelper x y n result = do
  if y `mod` 2 == 1
    then do
      let newres = ((result * x) `mod` n)
      let newy = quot y 2
      if newy == 0
        then newres
        else exMHelper ((x*x) `mod` n) newy n newres

    else do
      let newy = quot y 2
      if newy == 0
        then result
        else exMHelper ((x*x) `mod` n) newy n result
